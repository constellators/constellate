...
const toPackage = packagePath => {
  ...
  return {
    ...
    paths: {
      ...
      nodeModules: path.resolve(packagePath, './node_modules'),
      webpackCache: path.resolve(packagePath, './.webpackcache'),
    },
  }
}
...
function getPlugins(pkg) {
  return {
    buildPlugin: resolvePluginFor(pkg, 'build'),
    developPlugin: resolvePluginFor(pkg, 'develop'),
    deployPlugin: resolvePluginFor(pkg, 'deploy'),
  }
}
